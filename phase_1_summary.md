...
| Entity services               | `src/main/webapp/app/entities/professionalService/*/service/*.service.ts`                                                                                      | Reuse typed JHipster CRUD, REST date conversion, and `createRequestOption` patterns. `report.service.ts` currently targets `api/reports`; the approved clinical-report model remains separate pending backend reconciliation.  |
