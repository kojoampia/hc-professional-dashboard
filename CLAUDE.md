...
- Routing is standalone (`app.routes.ts`) with lazy `loadChildren` per area; route guards via `UserRouteAccessService` with `Authority` constants (`app/config/authority.constants.ts`). i18n is enabled (**en/es/fr/de** — Spanish was added for the careers handoff; note `.yo-rc.json` still lists only en/fr/de). See `AGENTS.md` for the three route authority tiers, including why `/onboarding` deliberately has no clinical-role guard. **Before adding or changing a translation, read `professional-web.md` §2 "Decisions that still bind"** — its terminology bullet carries the per-locale noun decisions no gate can enforce: es `expediente` for the clinical record with `historial` reserved for the activity log (and `expediente de acreditación` a third, qualified sense), de `Patientenakte`/`Akte` never `Datensatz`, fr `dossier` for the record and `cas` for the clinical case. (The proper nouns — the brand word `Professional`, `Ghana Card` — need no pointer: they are pinned in `brand-terms.spec.ts`, where a violation goes red naming its item.) One deliberate exception to know before "fixing" it: de `me.earnings.noRecord` says `Berufsdatensatz` on purpose — its referent is the clinician's **own** hc-admin row, not a patient record (`../docs/backlog.md` item 176).
...
- **Uniform font:** the whole application renders in Inter — the same face used (via inheritance, no per-component override) by the health-connect stat cards (`app/shared/health-connect/stat-card/stat-card.component.ts`). It is **self-hosted** — the faces live in `content/fonts/` and are declared in `content/scss/fonts.scss`, so nothing reaches fonts.googleapis.com and `font-src 'self'` is satisfiable. It is applied globally via `body { font-family: var(--hpd-font-body); }` in `content/scss/global.scss` (token defined in that same file's `:root` block), and also set as Tailwind's `--font-sans` in `content/css/tailwind.css` so the `font-sans` utility class agrees with it. Angular Material's M3 theme (`content/scss/material-theme.scss`) already specified `brand-family`/`plain-family: 'Inter'` — this is what makes that config actually render correctly instead of silently falling back to the browser default. Don't introduce a second font anywhere; if a component needs a different weight, use Tailwind's `font-*` weight utilities (`font-medium`, `font-semibold`, `font-bold`, …), not a different family.

## Docker / infrastructure

`src/main/docker/` holds compose files for dev dependencies the gateway expects in the wider microservice topology: `mongodb.yml`, `jhipster-registry.yml`, `kafka.yml`, `services.yml` (aggregates mongodb + registry + kafka). `npm run services:up` starts them. (MongoDB is the configured database type per `.yo-rc.json`.)

**The deployed image is not built here.** It comes from `../deploy/docker/web.Dockerfile` with this repo as the build context and its nginx config from `../deploy/docker/web-nginx.conf` — `../deploy/build.sh` on the `local` channel, `.github/workflows/release.yml` on ghcr. The root `docker-compose.yml` / `docker-compose-prod.yml` pair, `Dockerfile.dev`, `Dockerfile.prod` (with `Dockerfile` symlinked to it) and `nginx.conf` build a **local-only** image and nothing has shipped from them since August; they survive because `package.json`'s `docker:build:*` and `deploy:*` scripts still reach them. Change nginx behaviour in `../deploy/docker/web-nginx.conf`, not in this repo's `nginx.conf`. See `../docs/backlog.md` item 34.
