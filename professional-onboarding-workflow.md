...
| Careers: Spanish (2026-07-29)      | **Add Spanish now** — full `es` i18n catalog for the portal so `locale=es` works end to end (careers keeps offering Spanish).                                                                                                                        |
| Careers: role default (2026-07-29) | **No silent default** — the wizard's requested-role select starts empty and required; only an inbound `track` (or its stored copy) pre-selects. Kills the mislabeled-application hazard the contract flags.                                          |
| Careers: attribution (2026-07-29)  | **Queue + funnel count** — `src` persisted on the application, shown in the WP5 review queue and counted per source on the WP7 operational dashboard (careers task 145).                                                                             |
| Deployment (2026-07-29)            | **In scope** — new WP8 makes `professional.abofonsa.com` actually serve (build/deploy for gateway, api, web); careers flips its apply buttons only after WP8 + the handoff items are live.                                                           |
...
### Careers handoff (external contract) — added 2026-07-29

`../docs/careers-handoff-contract.md` (verified against `web@424a11f`, `api@25300a0`, `gateway@61d611c`) defines the inbound link from `web.abofonsa.com/careers`:

```
https://professional.abofonsa.com/register?track=<AuthorityRole>&locale=<en|es|fr|de>&src=web-careers
```

Exactly three parameters, never any personal data. Obligations and how this plan meets them:

1. **`track` must reach the onboarding wizard** across the anonymous-registration → email-activation → authenticated-onboarding boundary. Mechanism (receiver's call per the contract): the register page stores `{track, locale, src, at}` in **localStorage** (`hpd-careers-handoff`) when valid parameters are present — localStorage because the activation email opens a fresh tab, which kills sessionStorage; a cross-device activation loses the value gracefully. The wizard consumes and clears it: `track` pre-selects the requested role (values outside the known role set are ignored), `src` rides into the start-application call. Careers advertises six roles; the select still offers all nine.
2. **`locale`** sets the register form's `langKey` and switches the active UI language on arrival. Per the § Decisions row, a full **Spanish catalog** is added (`i18n/es/`, `LANGUAGES += 'es'`) so `locale=es` is honoured rather than mapped away.
3. **`src` attribution**: `ProfessionalApplication` gains a nullable `source` field set at creation (`StartApplicationRequest` gains it too; server accepts a short free string, truncated); surfaced as a queue column in WP5 and a per-source count on the WP7 dashboard.
4. **Graceful degradation stays**: absent/unknown parameters change nothing — the contract's Journey 9 asserts the sender's half; our specs/ITs assert this half.

Related fix from the same review: the wizard's requested-role control loses its `ROLE_NURSE` default — empty + required, so every applicant chooses explicitly.

The careers page keeps its apply buttons off (CMS `professionalPortalUrl` = null) until WP8 serves the portal **and** items 1–3 are live.
...
5. **WP4b — Careers handoff intake (`web/`, `api/`).** Register page reads `track`/`locale`/`src` (localStorage carry, langKey + language switch on arrival); wizard pre-selects from the stored track and sends `source`; requested-role default removed (empty + required); `ProfessionalApplication.source` + `StartApplicationRequest.source` on the api; full `es` i18n catalog and `LANGUAGES += 'es'`. _Gate:_ register-param specs (valid/unknown/absent), wizard preselect + no-default specs, api IT proving `source` persists, graceful degradation with a bare URL.
6. **WP5 — Reviewer/admin flow (`web/`, `api/`).** Review queue (data-table with status tints, **incl. the attribution/source column**), secure document preview (authorized streaming), per-document verify/reject, application approve/reject/return, organization assignment, authority assignment (gateway call), professional-access enablement. _Gate:_ role-gated e2e-style spec — reviewer vs admin vs applicant see disjoint affordances.
7. **WP6 — Duty roster + first login (`api/`, `web/`).** Generate the corrected `DutyRoster` entity (drop `patientId`, aligned enum); assignment-only admin UI; professional read-only roster view (swap the current mock repository path for the real API adapter); first-login acknowledgement interstitial writing `OnboardingEvent`s. _Gate:_ shift label in the sidebar user card driven by real assignments.
8. **WP7 — Compliance + operations.** Per-source application **funnel count** on the operational dashboard (careers task 145); scheduled expiry sweep over `PersonalDocument.expiryDate` (api scheduler + Kafka event or email hook), restriction/deactivation path, audit-history viewer for admins, operational dashboard tiles. _Gate:_ end-to-end test: expiring license → review task → restriction → reactivation after new document.
9. **WP8 — Deployment (`gateway/`, `api/`, `web/`).** Production build/containerization and compose (or equivalent) configuration so `professional.abofonsa.com` serves the gateway-fronted stack (web static assets + gateway TLS + api via Consul, MongoDB/Kafka provisioning, JWT secret and mail configuration). _Gate:_ the careers contract's verification commands pass against the live URL; only then does the careers CMS set `professionalPortalUrl` and switch the apply buttons on.
...
- **web:** `DutyRosterAssignmentsService` against the real surface (my/list/assign/unassign) with the **sidebar shift label computed from real assignments** (windows MORNING 06–14, AFTERNOON 14–22, NIGHT 22–06 incl. past-midnight; falls back to the next upcoming shift) — the mock `shiftLabelForAccount` path is gone from the shell. Duty-roster page reworked to assignment-only: professionals get a read-only list with an "assigned by administrators" notice; admins get an assign form (profile id/date/shift/duty/ward) plus unassign; the subscribe/unsubscribe UI is removed. First-login interstitial mounted in the main layout: shown only when the own application is **ACTIVE** and the acknowledgement is still pending (the api reports pending for any application, so the web gates on status), dismissed by the idempotent POST. i18n en/fr/de/es. 376/376 tests, lint + prod build green.
- Gate met: the sidebar user-card shift label is driven by real assignments (`duty-roster-assignments.service.spec` + sidebar spec), and the acknowledgement is a one-time audited event (`DutyRosterFlowIT`).
