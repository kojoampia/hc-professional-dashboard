...
- The app uses a single uniform font family, **Inter**, across the entire application — the same face the health-connect stat cards render in via inheritance. It is **self-hosted** (`content/fonts/`, declared in `content/scss/fonts.scss`) and applied globally on `body` in `src/main/webapp/content/scss/global.scss` (token `--hpd-font-body`), mirrored as Tailwind's `--font-sans` in `src/main/webapp/content/css/tailwind.css`, and matched by Angular Material's M3 theme config (`content/scss/material-theme.scss`). Do not introduce a second font family anywhere; use Tailwind's `font-*` weight utilities for emphasis instead.
...
The deployment bundle is **`../deploy/` at the workspace root**, not in this repo — it deploys all three repos, so it belongs to none of them. It **is its own git repository** (`hc-professional-ci`); the runbook is `../deploy/README.md`, the local stack is `../deploy/docker-compose.yml`, what ships to the server is `../deploy/prod-server/compose.yml`, and the one entry point is `../deploy/deploy.sh` — `--verify-only` runs the acceptance checks, and there is no separate `smoke.sh`. Only the web container publishes a port, `127.0.0.1:${FRONTEND_PORT:-5503}`.

**This repo supplies source, not deployment files.** The deployed dashboard image is built from **`../deploy/docker/web.Dockerfile`** with this repo as the build context, taking its nginx config from `../deploy/docker/web-nginx.conf` through a named build context (`--build-context deploycfg=./docker`). That is true on both channels: `../deploy/build.sh` locally, and this repo's own `.github/workflows/release.yml` on ghcr, which checks out `hc-professional-ci` for exactly that Dockerfile.

**The root `Dockerfile.prod`, the `Dockerfile` symlink to it, `Dockerfile.dev` and `nginx.conf` are not the deployment path** and have not been since August. They are kept only because `package.json`'s `docker:build:dev` / `docker:build:prod` / `deploy:{dev,prod}` scripts and the two root compose files still reach them — a local-image chain that also pushes to `docker-registry.jojoaddison.net`, a registry hostname that is not in use. **If you need to change nginx behaviour for the deployed app, change `../deploy/docker/web-nginx.conf`**; editing `nginx.conf` here changes nothing that ships. `../docs/backlog.md` item 34 tracks retiring the whole chain.

How prod API URLs work is the same in either image and worth knowing: `npm run webapp:prod` bakes `SERVER_API_URL='/'` (see `webpack/webpack.custom.js`), so in production every API call is **same-origin** and the container's nginx proxies the path set through to the gateway. The dev-time `webpack/proxy.conf.js` → `:5505` is a separate mechanism; changing the proxied path set means editing **both** it and `../deploy/docker/web-nginx.conf`.
