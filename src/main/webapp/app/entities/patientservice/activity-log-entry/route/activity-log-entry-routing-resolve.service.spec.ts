...
import activityLogEntryResolve from './activity-log-entry-routing-resolve.service';

describe('ActivityLogEntry routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IActivityLogEntry returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          activityLogEntryResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(activityLogEntryResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
