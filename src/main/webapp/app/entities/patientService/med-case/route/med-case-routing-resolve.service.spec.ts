...
import medCaseResolve from './med-case-routing-resolve.service';

describe('MedCase routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IMedCase returned by find', async () => {
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          medCaseResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(medCaseResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
