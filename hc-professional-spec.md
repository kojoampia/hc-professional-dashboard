...
5. **"Angel" terminology**: confirm real-world meaning (guardian, emergency contact, case manager) before building the field into production copy/labels. **Recommended backend modeling**: see §7.2 — a self-referential `Profile.contacts` relationship rather than flat name/phone text fields, so this can link through to the contact's own record.
...
14. **Backend/JDL alignment**: **partially resolved** — see §7 for a full entity/attribute extraction and draft JDL derived from the microservice architecture diagram and registration wireframe. Remaining open points are flagged inline in §7.2/§7.4 (Category's relationships, whether Credential/Option should be generated JDL entities or map onto JHipster's built-in User, and whether Document should be shared between registration documents and clinical Reports).
...
## 7. Backend Entity Model (JDL Basis)

> Source: microservice architecture diagram (bounded contexts `Microservice` and `Gateway`) + `HC-Register-Health-Professional.png` registration wireframe. This section resolves §4 Gap #14 (backend/JDL alignment) by extracting concrete entity/attribute definitions an agent can turn directly into a JHipster JDL file. Types below are inferred/recommended JDL primitive types, not shown in the source images — confirm before generation.

### 7.1 Bounded Contexts

Two JHipster applications/contexts are implied by the diagram:

- **`microservice`** — domain entities: `Profile`, `Address`, `Document`, `Category`, `Activity`, `Team`, `Roster`.
- **`gateway`** — auth/routing-adjacent entities: `Credential`, `Option`. This is presumed to be the JHipster gateway app itself (API routing + the account/auth layer), consistent with §0.2's note that JHipster's built-in account module already exists — **`Credential` likely represents JHipster's own `User`/account entity rather than a new custom entity; confirm before modeling it as a separate JDL entity** to avoid duplicating JHipster's built-in user table.
...
| sex | `Sex` (enum) | See §7.3 |
| mobilePhone | `String required` | |
| email | `String required` | |
| idType | `IdentificationType` (enum) | See §7.3; matches the registration wireframe's "Identification Type" dropdown (`Passport`, `Ghana Card`) |
| idNumber | `String` | matches wireframe's "Identification Number" |
| title | `String` | e.g. professional title/designation — confirm exact meaning before build |

**Relationships:**
- `Profile` → `Address` (one-to-one, field `address`)
- `Profile` → `Profile` (many-to-many, field `contacts`) — self-referential. **This is the recommended resolution for §4 Gap #5** (the "Angel" contact field on the Patient Identity Panel, §3.3): model the Angel/guardian/next-of-kin relationship as a `Profile.contacts` self-reference rather than a flat name/phone pair, so the frontend can eventually link through to that contact's own `Profile` record instead of duplicating their name/phone as plain text.
...
No relationships shown; owned one-to-one by `Profile`.
...
| description | `String` | |
| url | `String` | storage location of the attached file — pair with a JHipster `blob`/`anyblob` field if files are stored in-database rather than object storage |
| timestamp | `Instant` | |
| documentType | `DocumentType` (enum) | New field, not in the diagram's `Document` box but required by the registration wireframe's "Add a document" dialog (`Certificate`, `Passfoto`, `Passport`, `Ghana Card`, `Other`); see §7.3 |
| otherTypeLabel | `String` | populated only when `documentType = OTHER`, from the wireframe's "Other:" free-text field |

**Relationship:** `Document` → `Profile` (many-to-one) — a professional's uploaded documents belong to their `Profile`; not explicit in the diagram but required by the registration flow. This also maps directly onto the frontend's Reports panel (§3.3, `FileUploadTriggerComponent`) and Patient Detail Record Documents concept — confirm whether `Document` should be reused for both the professional's own registration documents and a patient's clinical Reports, or whether these should be two distinct entities.
...
No relationships shown in the diagram; likely referenced by `Option.category` (Gateway) and/or used to classify `Document`/`Activity` records — confirm exact usage before finalizing relationships.
...
**Relationship:** `Activity` → `Profile` (many-to-one, field `patient`) — matches the frontend's Visitations/Activity Trail panels (§3.3).
...
**Relationship:** `Team` → `Profile` (many-to-one, field `supervisor`) — the team's supervising professional.
...
| description | `String` | |
| date | `LocalDate` | |
| duration | `Duration` | JHipster's `Duration` JDL type, or `Integer` (minutes) if simpler |

**Relationship:** `Roster` → `Profile` (many-to-one, field `patient`).

**Cross-reference to §1.2 `DutyRoster`:** this diagram's `Roster` entity is named similarly to, but is **not the same concept as**, the frontend spec's `DutyRoster`/`DutyShift` (§1.2) — this `Roster` is patient-scoped (a patient's scheduled activity/duration), while `DutyRoster` is professional-scoped (a clinician's on-call schedule). Keep both; do not merge them. Confirm naming with the team to avoid confusion between "Roster" (patient) and "Duty Roster" (professional) before generating JDL, since a generated `Roster` entity and a hand-modeled `DutyRoster` entity sitting side by side will read as redundant to anyone unfamiliar with this distinction.
...
| email | `String required` | |
| phoneNumber | `String` | |
| password | `String required` | **if this app uses JHipster's built-in account/`User` entity (per §0.2), do not model a separate `password` field here** — JHipster already handles credential storage/hashing; this box in the diagram may simply be documenting JHipster's existing `User` shape rather than requesting a new entity. Confirm before generating JDL for this entity. |
| role | `AuthorityRole` (enum) | **Directly reuses the `AuthorityRole` enum already defined in §1.1** (`Doctor`, `Nurse`, `Paramedic`, `Pharmacist`, `Therapist`, `Carer`, `Admin`, `User`) — do not redefine a separate role enum for `Credential`; this is the same authority concept, and should follow the JHipster authority-mapping table in §1.1, not a plain string field. |

#### `Option` (gateway)
| Field | Inferred type | Notes |
|---|---|---|
| category | `String` | possibly should be a relationship to `Category` (microservice) rather than a plain string — cross-microservice relationships aren't directly supported in a single JDL the way in-context ones are, so if `gateway` and `microservice` are separate JHipster apps, this likely needs to stay a plain string/reference-by-name (or reference-by-id) rather than a JDL `relationship` block. Flag for architecture review. |
| userId | `String` | reference to the owning `Credential`/JHipster `User` — same cross-context caveat as above |
| metadata | `TextBlob` or `String` | freeform; recommend `TextBlob` if this can hold structured/JSON data |
...
enum DocumentType {
  CERTIFICATE, PASSFOTO, PASSPORT, GHANA_CARD, OTHER
}
```

`AuthorityRole` (reused from §1.1, not redefined here):
```
enum AuthorityRole {
  DOCTOR, NURSE, PARAMEDIC, PHARMACIST, THERAPIST, CARER, ADMIN, USER
}
```
...
relationship ManyToMany {
  Profile{contacts} to Profile
}

relationship ManyToOne {
  Document{owner} to Profile
  Activity{patient} to Profile
  Team{supervisor} to Profile
  Roster{patient} to Profile
}

// Category is left unattached pending confirmation of its exact relationships (§7.2)
// Credential and Option (gateway context) are intentionally omitted from this JDL block —
// see §7.2 notes on JHipster's built-in User entity and cross-context relationship caveats
// before modeling them as generated entities.
```

**Before running this JDL through the entity sub-generator**, resolve the open confirmation points flagged inline above (`Category`'s relationships, whether `Credential`/`Option` should be generated entities or map onto JHipster's existing `User` and a lightweight settings/preferences shape, and the `Document` reuse question between registration documents and clinical Reports).
