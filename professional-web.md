...
| **One raw hex left on the roster page**                   | `duty-roster-page.component` uses `bg-[#e7eef6]` on the admin estate list's shift badge, against the token rule above. A navy-tint token is the right fix, but the obvious value sits 1.07 from DR5's sick tint, so naming them apart is its own decision. Left for DR8, which rebuilds that block.                                                                                                      |
